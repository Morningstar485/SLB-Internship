...
class Prepare_MRPC_9(QDialog):
    ...
    def next_step(self):
        ...
        if self.ui.radioButton_1.isChecked() and self.ui.radioButton_3.isChecked() and self.ui.radioButton_5.isChecked():
            from .Prepare_MRPC_10 import Prepare_MRPC_10
            dialog = Prepare_MRPC_10(self.child_metadata, self.admin_metadata)
            if dialog.exec() == QDialog.Accepted:
                self.accept()  # ✅ Close this dialog and continue chain
        else:
            from .Prepare_MRPC_11 import Prepare_MRPC_11
            dialog = Prepare_MRPC_11(self.child_metadata, self.admin_metadata)
            if dialog.exec() == QDialog.Accepted:
                self.accept()  # ✅ Close this dialog and continue chain